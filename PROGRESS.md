...
|----------|--------|-----------|
| Build system | Astro 7 (static) | Component-based, MDX for case studies, fast GitHub Pages builds |
...
| 2026-06-10 | Bug fixes (#45 #46 #47 #48): stream filter + contact form re-bind on `astro:page-load` (were dead after view-transition navigation), per-page localized titles/descriptions for Home and Stream, reveal animation gated on a `.js` class (content no longer invisible without JS), SV stream fallback gets `lang="en"` content markup + canonical to EN original + visible notice; theme now re-applied on `astro:after-swap` |
| 2026-06-10 | Privacy #49: self-hosted Inter via @fontsource (Google Fonts removed — no visitor IPs to Google, GDPR-safe) |
| 2026-06-10 | A11y (#55 #56): `prefers-reduced-motion` support (reveal/pulse/scroll/view-transition animations disabled), `aria-pressed` on stream filter buttons, theme/menu toggle aria-labels via i18n |
...
| 2026-07-10 | Upgraded to Astro 7; added SAAB IT Object Leader role; introduced secure issue-form content publishing through validated pull requests; strengthened keyboard accessibility, contact privacy, CSP, CI, and repository documentation |
